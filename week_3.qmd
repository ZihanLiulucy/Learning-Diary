...
![](geometric_correction.png)

Figure from [@DaveEtAl2015]
...
![](week_3_workshop_2.png){width="315"} ![](week_3_workshop_3.png){width="270"}

The figure on the left hand side shows the NDVI map in York, we can distinguish the water, land and greenery on the map. The NDVI value of water is less than 0.2, while land is a little bit higher, and the greenery is the highest, with the color of green shown on the map, NDVI is higher than 0.4.

The basic logic is that Water bodies usually have very low NDVI values because water absorbs most of the sunlight in the visible and near-infrared wavelengths, leading to negative NDVI values or values close to zero. Land often contains some vegetation, so the value would be higher than water, and values close to +1 suggest dense and healthy green vegetation as these areas are reflecting more infrared and less visible lights.

The right hand side figure shows the points where NDVI is higher than 0.2, filtering out most water bodies, the remained textures are mainly solid.
...
![](geometric_application.jpg)

Figure from [@Rocchini2005]
